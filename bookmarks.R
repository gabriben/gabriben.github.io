...
library(rjson)
f <- fromJSON(file = "/Users/gabrielbenedict/Google_Drive/docs/UNIS/KU Leuven/Exchange/columbia/Courses/Applied Data Science/Projects/gabriben.github.io/bookmarks.json")


library(data.table)

g <- rbindlist(f$roots$bookmark_bar$children[c(-1,-2)], fill=TRUE)

str(f$roots$bookmark_bar$children[c(-1,-2)])

dfs <- flattenList(f$roots$bookmark_bar$children[c(-1,-2)])

dfs[,grep("type", colnames(dfs)) ] 
